...
describe('Single signer pdf signing', () => {
  it('Signs a pdfs using a single signer', () => {
    ...
    //enter email
    cy.get('#email',{ timeout : 5000 }).type(email)
    ...
    cy.get('[id^="1"]',{ timeout : 5000 }).then(($object) => {
          // Resize the object
    ...
    cy.get('input',{ timeout : 5000 }).type('Signer')
    ...
    cy.get('.pl-2 > :nth-child(2) > .mat-focus-indicator',{ timeout : 5000 }).click()

    cy.get('.mat-stroked-button',{ timeout : 5000 }).click()

    cy.get(':nth-child(3) > .mat-focus-indicator',{ timeout : 5000 }).click()
  })
})